...
def load_knowledge_base() -> int:
    """扫描 knowledge 目录（含子目录），将 Markdown 文档分块后向量化入库。"""
    ...
    for md_file in sorted(KNOWLEDGE_DIR.rglob("*.md")):
        # 用"子目录名/文件名"作为 spot_name，便于区分
        rel_path = md_file.relative_to(KNOWLEDGE_DIR)
        spot_name = str(rel_path.with_suffix("")).replace("\\", "/")
        content = md_file.read_text(encoding="utf-8")
        chunks = [c.strip() for c in content.split("\n\n") if c.strip()]

        ids = [f"{spot_name}_chunk_{i}" for i in range(len(chunks))]
        metadatas = [{"spot_name": spot_name, "chunk_index": i, "category": rel_path.parts[0] if len(rel_path.parts) > 1 else "城市"} for i in range(len(chunks))]
