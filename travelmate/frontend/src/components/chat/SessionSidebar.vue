...
<template>
  <aside class="flex h-full w-64 flex-col bg-white text-stone-700 border-r border-stone-200 dark:bg-[#212121] dark:text-stone-300 dark:border-stone-700">
    ...
    <div class="flex items-center justify-between px-3 pt-3 pb-2">
      <button
        class="rounded-lg p-1.5 text-stone-400 transition-colors hover:bg-stone-100 hover:text-stone-600 dark:hover:bg-[#2f2f2f] dark:hover:text-stone-200"
        title="收起侧边栏"
    ...
    <div class="px-3 pb-2">
      <button
        class="flex w-full items-center gap-2 rounded-lg border border-stone-200 px-3 py-2 text-sm text-stone-700 transition-colors hover:bg-stone-100 dark:border-stone-600 dark:text-stone-200 dark:hover:bg-[#2f2f2f]"
        @click="store.createSession()"
      >
    ...
    <div class="flex-1 overflow-y-auto px-2 py-1">
      <div v-if="store.sessions.length === 0" class="px-3 py-8 text-center text-xs text-stone-400 dark:text-stone-500">
        暂无会话
      </div>
      ...
      <div
        ...
        :class="store.sessionId === s.session_id
          ? 'bg-stone-100 text-stone-800 dark:bg-[#2f2f2f] dark:text-stone-100'
          : 'text-stone-500 hover:bg-stone-100 hover:text-stone-700 dark:text-stone-400 dark:hover:bg-[#2a2a2a] dark:hover:text-stone-200'"
      ...
      >
        ...
        <div v-if="renamingId === s.session_id" class="flex flex-1 items-center gap-1">
          <input
            ...
            v-model="renameInput"
            class="min-w-0 flex-1 rounded border border-stone-300 bg-white px-1.5 py-0.5 text-sm text-stone-800 outline-none ring-1 ring-stone-300 dark:border-stone-500 dark:bg-[#171717] dark:text-stone-100 dark:ring-stone-500"
            @keydown="handleRenameKeydown($event, s.session_id)"
            @blur="commitRename(s.session_id)"
            @click.stop
          />
          <button
            class="shrink-0 rounded p-0.5 text-stone-400 hover:text-stone-600 dark:hover:text-stone-200"
            title="确认"
        ...
        <template v-else>
          ...
          <div class="flex shrink-0 items-center gap-0.5 opacity-0 transition-all group-hover:opacity-100">
            <button
              class="rounded p-0.5 text-stone-400 hover:text-stone-600 dark:text-stone-500 dark:hover:text-stone-200"
              title="重命名"
            ...
            <button
              class="rounded p-0.5 text-stone-400 hover:text-red-400 dark:text-stone-500 dark:hover:text-red-400"
              title="删除"
