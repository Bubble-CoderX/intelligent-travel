...
- [十一、行程风格对比](#十一行程风格对比)
  ...
  - [11.2 风格定义](#112-风格定义)
  - [11.3 数据流](#113-数据流)
  - [11.4 后端改动详情](#114-后端改动详情)
  - [11.5 前端改动详情](#115-前端改动详情)
  - [11.6 设计决策：风格选择时机](#116-设计决策风格选择时机)
  - [11.7 验证方法](#117-验证方法)
  - [11.8 改造涉及文件清单](#118-改造涉及文件清单)
- [十二、首页天气显示](#十二首页天气显示)
- [十三、会话启动主动问候](#十三会话启动主动问候)
- [附录B：P3 验证与限制](#附录bp3-验证与限制)
...
| `backend/app/api/knowledge.py` | **新增** | `/auto-expand` + `/has-local` + `/auto-expand-batch` 端点 |
| `backend/app/services/rag_service.py` | 小改 | 检索为空时自动触发 auto_expand + 新增 `_extract_spot_name()` 辅助函数 |
| `backend/app/services/intent_router.py` | 小改 | 新增 CORRECTION 检测 + 触发纠正流程 |
| `backend/requirements.txt` | 小改 | 新增 `duckduckgo-search>=6.0.0` |
| `backend/app/main.py` | 小改 | 注册 knowledge_router |
| `frontend/src/components/chat/ChatContainer.vue` | 小改 | 展示批量扩充的 SSE 进度条 |
| `frontend/src/components/chat/MessageBubble.vue` | 小改 | "🔍 已自动调研"绿色标签 |
...
### 10.6.9 实现细节：knowledge_expander.py 函数表
...
| 文件 | 改动类型 | 说明 |
|------|----------|------|
| `backend/app/models/schemas.py` | 字段新增 | ChatRequest 新增 `trip_style` 字段 |
| `backend/app/utils/trip_prompts.py` | 新增常量 + 改造 | 新增 `STYLE_INSTRUCTIONS` + prompt 占位符 |
| `backend/app/services/trip_service.py` | 函数签名扩展 | `generate_trip_plan()` 新增 `style` 参数 |
| `backend/app/services/intent_router.py` | 参数透传 | `route_intent()` 透传 `trip_style` |
| `backend/app/api/chat.py` | 参数透传 + metadata | 传递 `trip_style` + metadata 增强 |
| `frontend/src/components/chat/StyleSelector.vue` | **新建** | 行程风格选择器组件 |
| `frontend/src/components/chat/ChatInput.vue` | 逻辑新增 | 旅行关键词检测 + StyleSelector |
| `frontend/src/stores/chat.ts` | 函数签名扩展 | `sendMessage()` 新增 `tripStyle` 参数 |
| `frontend/src/components/chat/ChatContainer.vue` | 逻辑新增 | 风格切换逻辑 + StyleSelector 面板 |
| `frontend/src/components/chat/TripCard.vue` | Props 扩展 | 风格标签 + "换种风格"按钮 |
